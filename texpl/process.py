...
def run(command: List[str], queue='default', stdin=None, cwd=None, env={}, stream_reader=None, stop_token=None, ignore_errors=False, encoding='utf-8', fallback_encoding=[]):
    ...
    return worker_run(partial(job, command, queue, stdin, cwd, environment, stream_reader, ignore_errors, encoding, fallback_encoding, task_id), queue, task_id=task_id)
...
def get_output_streamed(command: List[str], stream_reader, stop_token=None, ignore_errors=False, success_codes=[0], *args, **kwargs):
    if stop_token is None:
        stop_token = threading.Event()

    error_code, _, _ = run(command, *args, stream_reader=stream_reader, stop_token=stop_token, ignore_errors=ignore_errors, **kwargs)
    if not ignore_errors and error_code not in success_codes:
        command_str = ' '.join(command)
        raise JobError(f'Error when executing command "{command_str}" (exit code {error_code}).')
